...
type DadosRescisao = z.infer<typeof FormSchema>;

// --- FUNÇÕES DE VALIDAÇÃO ---

function validarDados(dados: DadosRescisao): void {
  if (!dados) {
    throw new Error("Objeto de dados da rescisão não pode ser nulo.");
  }
  const { dataInicio, dataFim, salarioBruto } = dados;
...
function calcularMesesTrabalhados(dataInicio: Date, dataFim: Date): number {
  if (isAfter(dataInicio, dataFim)) {
    return 0;
  }
...
function calcularDiasAvisoPrevio(dataInicio: Date, dataFim: Date): number {
...
function calcularINSS(baseCalculoEmCentavos: number): number {
  let inssTotal = 0;
  ...
  for (let i = 0; i < TABELA_INSS.length; i++) {
    ...
    const tetoFaixa = faixa.ate - limiteAnterior;
...
function calcularIRRF(
  baseCalculoEmCentavos: number,
  inssEmCentavos: number,
  dependentesIR: number,
  usarDeducaoDependente: boolean = true
): number {
...
function calcularPensaoAlimenticia(
  baseCalculoEmCentavos: number,
  percentual: number
): number {
...
function formatarParaBRL(valorEmCentavos: number): string {
...
export function calcularRescisaoCompleta(dados: DadosRescisao) {
  validarDados(dados);
  ...
  const salarioBrutoEmCentavos = salarioBruto * 100;
